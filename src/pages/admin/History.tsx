...
const History: React.FC = () => {
    ...
    const fetchHistory = async () => {
        ...
        try {
            ...
            console.error("❌ [History] Error details:", error.response?.data || error.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const getAdminExpenseTotal = (car: SoldCar) => {
        return car.adminExpenses?.reduce(
    ...
    const getReceivedAmount = (car: SoldCar) => {
        if (!car.paymentSummary) return 0;
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-gray-600">
                        Complete details of all sold cars
                    </p>
                </div>
                ...
                {!loading && history.length > 0 && (
                    <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
                        ...
                        {history.map((car) => {
                            ...
                            return (
                                <Card key={`${car._id}-${car.source}-${car.saleId || ""}`} className="overflow-hidden hover:shadow-lg transition-shadow">
                                            ...
                                            <div className="bg-white border rounded-lg p-4">
                                                ...
                                                {car.payments.map((p, i) => (
                                                    <div key={i} className="flex justify-between text-sm border-b py-1">
                                                        <span>
                                                            ₹{p.amount.toLocaleString()} ({p.paymentMode || p.paymentType})
                                                        </span>
                                    ...
                                    {(() => {
                                        const avgMargin =
                                            history.reduce((sum, car) => {
                                                ...
                                                return sum + (car.sellerPrice ? (net / car.sellerPrice) * 100 : 0);
                                            }, 0) / history.length;
